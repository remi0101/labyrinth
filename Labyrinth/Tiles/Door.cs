...
namespace Labyrinth.Tiles
{
    ...
    public class Door : Tile
    {
        ...
        public async Task<bool> Open(LocalInventory keySource)
        {
            if (IsOpened)
            {
                throw new InvalidOperationException("Door is already unlocked.");
            }
            await LocalInventory.MoveFirst(keySource);
            if (LocalInventory.Items.First() != _key)
            {
                await keySource.MoveFirst(LocalInventory);
            }
            return IsOpened;
        }
        ...
        public async Task LockAndTakeKey(LocalInventory whereKeyGoes)
        {
            if (IsLocked)
            {
                throw new InvalidOperationException("Door is already locked.");
            }
            await whereKeyGoes.MoveFirst(LocalInventory);
        }
