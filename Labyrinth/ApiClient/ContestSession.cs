...
namespace Labyrinth.ApiClient
{
    ...
    public class ContestSession
    {
        ...
        public static Task<ContestSession> Open(Uri serverUrl, Guid appKey, Dto.Settings? settings = null)
        {
            var http = new HttpClient() { BaseAddress = serverUrl };
            return Task.FromResult(new ContestSession(http, appKey));
        }
        ...
        {
            var crawlerDto = await CreateCrawler(_http, _appKey);
            var (crawler, bag) = NewCrawlerAndItsBag(_appKey, crawlerDto);
            _crawlers.Add((crawler, bag));

            // Initialiser le builder avec le premier crawler
            if (_callsToNewCrawler == 0)
            {
                _builder = new RemoteContestLabyrinthBuilder(crawler);
            }
        ...
        /// <summary>
        /// Crée N crawlers en une seule fois.
        /// </summary>
        public async Task<List<ICrawler>> CreateCrawlers(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                try
                {
                    var crawler = await NewCrawler();
                    crawlers.Add(crawler);
                }
                catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.Forbidden)
                {
                    Console.WriteLine($"Cannot create crawler {i + 1}/{count}: limit reached");
                    break;
                }
            }
        ...
        private ContestSession(HttpClient http, Guid appKey)
        {
            _http = http;
            _appKey = appKey;
        ...
        private static async Task<Dto.Crawler> CreateCrawler(HttpClient http, Guid appKey,
        ...
        private RemoteContestLabyrinthBuilder _builder;
